...
class _RemoteFootwearShowcase extends StatelessWidget {
  const _RemoteFootwearShowcase({
    required this.items,
  ...
  Widget build(BuildContext context) {
    return _FootwearInfiniteFeedSection(
      items: items
          .map(
            (item) => _FootwearFeedCardData(
              item: item.item,
...
class _RemoteFootwearShopPage extends StatefulWidget {
  const _RemoteFootwearShopPage({
  ...
  @override
  State<_RemoteFootwearShopPage> createState() => _RemoteFootwearShopPageState();
}

class _RemoteFootwearShopPageState extends State<_RemoteFootwearShopPage> {
  _FootwearShopProfileData? _profile;
  ...
  Future<void> _loadProfile({int? categoryId}) async {
    ...
    try {
      final profile = await _UserAppApi.fetchFootwearShopProfile(
        widget.shopId,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Could not load footwear shop right now.')),
      );
    }
  }
  ...
  Future<void> _selectCategory(String value) async {
    ...
    int? categoryId;
    for (final item in _profile?.categories ?? const <_FootwearRemoteCategory>[]) {
      if (item.label == value) {
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : profile == null
              ? const Center(
                  child: Text(
                    'Footwear shop is not available right now.',
                    style: TextStyle(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  profile.city.isEmpty ? 'Footwear shop' : profile.city,
                                  style: const TextStyle(
