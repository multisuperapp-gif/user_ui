...
class _GroceryProfilePage extends StatefulWidget {
  const _GroceryProfilePage({
    required this.item,
  ...
  @override
  State<_GroceryProfilePage> createState() => _GroceryProfilePageState();
}

class _GroceryProfilePageState extends State<_GroceryProfilePage>
    with _ScrollToTopVisibilityMixin<_GroceryProfilePage> {
  late String _selectedCategory;
  String _sortOption = 'Popular';
  bool _didAutoAdd = false;
  final ScrollController _scrollController = ScrollController();
  late final List<_GroceryMenuSectionData> _sections;
  ...
  void initState() {
    ...
    _selectedCategory = _groceryCategoryOptions.contains(widget.initialCategory)
        ? widget.initialCategory
        : 'Biscuits';
    _sections = _buildSectionsForShop(widget.item.subtitle);
    _sectionKeys = {
      for (final section in _sections) section.title: GlobalKey(),
    };
    _itemKeys = {
      for (final item in _sections.expand((section) => section.items)) item: GlobalKey(),
    };
  }
  ...
  List<_GroceryMenuSectionData> get _visibleSections {
    final filtered = _selectedCategory == 'All'
        ? _sections
        : _sections.where((section) => section.title == _selectedCategory).toList();
    return filtered
        .map(
          (section) => _GroceryMenuSectionData(
            title: section.title,
            items: _sortedGroceryItems(section.items, _sortOption),
          ),
        )
        .toList();
  }
  ...
  List<_DiscoveryItem> get _recommendedItems {
    return _visibleSections
        .expand((section) => section.items.take(2))
        .take(4)
        .toList();
  }

  List<_GroceryMenuSectionData> _buildSectionsForShop(String shopName) {
    return [
      for (final category in const ['Biscuits', 'Noodles', 'Rice', 'Chips', 'Household'])
        _GroceryMenuSectionData(
          title: category,
          items: _groceryItemsForCategory(category, shopName: shopName),
        ),
    ];
  }
  ...
  Future<void> _openFilterPage() async {
    await Navigator.of(context).push<bool>(
      MaterialPageRoute<bool>(
        builder: (_) => _ShopGroceryFilterPage(
          initialCategory: _selectedCategory == 'All' ? 'Biscuits' : _selectedCategory,
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final shopTiming = _shopTimingFor(shopName, 'Groceries');
        ...
        children: [
          ListView(
            ...
            children: [
              ...
              const SizedBox(height: 14),
              _GroceryProfilePromoStrip(
                items: _recommendedItems,
                onTap: _openItemPage,
              ),
              const SizedBox(height: 18),
              for (final section in visibleSections) ...[
                _GroceryProfileSectionHeader(
                  key: _sectionKeys[section.title],
                  title: section.title,
                  subtitle: _groceryCampaignTitle(section.title),
                ),
                const SizedBox(height: 10),
                GridView.builder(
                  shrinkWrap: true,
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _GroceryProfileItemCard(
                      key: _itemKeys[item],
                      item: item,
