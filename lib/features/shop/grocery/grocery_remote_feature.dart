...
class _RemoteGroceryShowcase extends StatelessWidget {
  const _RemoteGroceryShowcase({
    required this.items,
  ...
  List<_GroceryCampaignSectionData> get _sections {
    ...
    return labels
        ...
        .map(
          (label) => _GroceryCampaignSectionData(
            category: label,
            title: _groceryCampaignTitle(label),
            subtitle: _groceryCampaignSubtitle(label),
            colors: _groceryCampaignColors(label),
...
class _RemoteGroceryShopPage extends StatefulWidget {
  const _RemoteGroceryShopPage({
  ...
  @override
  State<_RemoteGroceryShopPage> createState() => _RemoteGroceryShopPageState();
}

class _RemoteGroceryShopPageState extends State<_RemoteGroceryShopPage> {
  _GroceryShopProfileData? _profile;
  ...
  Future<void> _loadProfile({int? categoryId}) async {
    ...
    try {
      final profile = await _UserAppApi.fetchGroceryShopProfile(widget.shopId, categoryId: categoryId);
      if (!mounted) return;
      setState(() {
        _profile = profile;
        _loading = false;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Could not load grocery shop right now.')),
      );
    }
  }
  ...
  Future<void> _selectCategory(String value) async {
    ...
    int? categoryId;
    for (final item in _profile?.categories ?? const <_GroceryRemoteCategory>[]) {
      if (item.label == value) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _loading
          ? const Center(child: CircularProgressIndicator(color: Color(0xFF2E8E45)))
          : profile == null
              ? const Center(child: Text('Grocery shop is not available right now.'))
              : ListView(
