...
class _PharmacyItemDetailPage extends StatefulWidget {
  const _PharmacyItemDetailPage({
    required this.item,
  ...
  @override
  State<_PharmacyItemDetailPage> createState() => _PharmacyItemDetailPageState();
}

class _PharmacyItemDetailPageState extends State<_PharmacyItemDetailPage> {
  int _quantity = 1;
  bool _addedInSession = false;
  String _pack = '1 Pack';
  static const List<String> _packs = ['1 Pack', '2 Packs', 'Value Pack'];

  int get _basePrice {
    final amount = _extractAmount(widget.item.price);
    return amount <= 0 ? 59 : amount;
  }

  int get _mrp {
    final calculated = (_basePrice * 1.3).round();
    return calculated <= _basePrice ? _basePrice + 20 : calculated;
  }

  int get _discountPercent {
    if (_mrp <= 0 || _mrp <= _basePrice) return 0;
    final percent = (((_mrp - _basePrice) / _mrp) * 100);
    if (!percent.isFinite || percent.isNaN) return 0;
    return percent.round();
  }

  int get _packDelta => switch (_pack) {
        '2 Packs' => _basePrice,
        'Value Pack' => _basePrice * 2,
        _ => 0,
      };

  int get _finalPrice => (_basePrice + _packDelta) * _quantity;
  _ShopTimingState get _shopTiming => _shopTimingFor(widget.item.subtitle, 'Pharmacy');
  ...
  void _closeWithResult() {
    Navigator.of(context).pop(widget.returnToShopOnBackAfterAdd && _addedInSession);
  }
            ...
            Container(
              ...
              child: Row(
                children: [
                  const Expanded(
                    child: Text(
                      'Pharmacy deal\nShop trusted daily healthcare essentials with nearby stock.',
                      style: TextStyle(
            ...
            Container(
              ...
              child: const Column(
                ...
                children: [
                  ...
                  SizedBox(height: 8),
                  Text(
                    'Quick doorstep delivery from nearby pharmacy. Keep away from direct sunlight and use as directed on pack.',
                    style: TextStyle(
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: const Color(0xFF8ACBD8)),
                  ),
                  child: Row(
                    ...
                    children: [
                      InkWell(
                        ...
                        child: const Icon(Icons.remove_rounded, color: Color(0xFF268B9C)),
                      ),
                      Text(
                        '$_quantity',
                        style: const TextStyle(
                      ...
                      InkWell(
                        ...
                        child: const Icon(Icons.add_rounded, color: Color(0xFF268B9C)),
                      ),
                    ],
                  ),
                ),
